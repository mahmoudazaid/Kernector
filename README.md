...
messages, draft, runStatus, requestStartedAt, unread); `kernector:active-session:v1`
stores only `activeConversationId`. `/chat` is a landing page (empty composer +
Chats list — never a transcript); submitting creates a thread, sets
`runStatus: pending`, navigates to `/chat/{id}`, and runs the ask through a
client-side conversation run coordinator keyed by that id so late responses
always append to the originating conversation. Pending/unread/failed status
shows on Chats rows; opening a thread clears unread. Use the main Chat nav item
to return to the landing page. Legacy `kernector:chat-messages:v1` (and pre-#246
session payloads with messages) are migrated once into the conversations store.
